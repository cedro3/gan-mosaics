...
class Mosaic:
    ...
    def generate_randomly(self, truncation_psi = 0.7):
        return self.generate_images_from_seeds(np.random.randint(4294967295, size=1), truncation_psi=truncation_psi)

    def generate_manual(self, seed, truncation_psi = 0.7):
        print(seed)
        return self.generate_images_from_seeds(seed, truncation_psi=truncation_psi)
    ...
    def generate_grid_manual(self, seeds, truncation_psi = 0.7):
        return create_image_grid(self.generate_images(self.generate_zs_from_seeds(seeds), truncation_psi), 0.7 , 3)    
    
    #def generate_animation(self, size = 9, steps = 10, trunc_psi = 0.7):
    def generate_animation(self, size=9, steps =10, trunc_psi = 0.7):
    ...
    #def generate_animation_manual(self, seeds, steps = 10, trunc_psi = 0.7):
    def generate_animation_manual(self, seeds, steps =10, trunc_psi = 0.7):
        seeds = seeds + [seeds[0]]
        zs = self.generate_zs_from_seeds(seeds)
